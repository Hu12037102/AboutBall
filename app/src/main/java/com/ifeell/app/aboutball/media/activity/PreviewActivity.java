...
import com.ifeell.app.aboutball.media.OnRecyclerItemClickListener;
import com.ifeell.app.aboutball.media.adapter.MediaCheckAdapter;
import com.ifeell.app.aboutball.media.adapter.PreviewAdapter;
  ...
  /*  @Override
    ...
    protected void initData() {
        ...
        if (mMediaFileData == null || mMediaFileData.size() == 0) {
            Toasts.with().showToast("没有预览媒体库文件");
            finish();
            return;
        }
            ...
            public void onSureClick(@NonNull View view) {
               /* if (hasFileCheckVideo()) {
                ...
                if (mCheckMediaData.size() < mOptions.maxChooseMedia || (mCheckMediaData.size() == mOptions.maxChooseMedia && mMediaFileData.get(mPreviewPosition).isCheck)) {
                    ...
                    Toasts.with().showToast(R.string.max_choose_media, String.valueOf(mOptions.maxChooseMedia));
                }
        ...
        if (mOptions.isCompress && !mOptions.isCrop) {
            ...
            if (mOptions.isCrop && mOptions.maxChooseMedia == 1) {
                if (!mCheckMediaData.get(0).isVideo) {
                    ...
                    Toasts.with().showToast(R.string.video_not_crop);
                }
            } else {
                EventBus.getDefault().post(mCheckMediaData);
                finish();
    ...
    private void titleViewAnimation() {
        ...
        if (isShowTitleView) {
            ...
            mStatusBarManger.setStatusBarTintEnabled(false);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (resultCode) {
                ...
                if (requestCode == UCrop.REQUEST_CROP) {
                    ...
                    if (resultUri != null && resultUri.getPath() != null) {
                        ...
                        if (FileUtils.existsFile(file.getAbsolutePath())) {
                            ...
                            Toasts.with().showToast(R.string.file_not_exit, Toast.LENGTH_SHORT);
                        }
                    }
            ...
            case UCrop.RESULT_ERROR:
                if (requestCode == UCrop.REQUEST_CROP) {
                    Toasts.with().showToast(R.string.crop_image_fail);
                }
                break;
            default:
                break;
